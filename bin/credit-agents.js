...
function supportsAvx2() {
  ...
  if (process.env.CREDIT_AGENTS_FORCE_BASELINE === "1") {
    return false;
  }
...
function main() {
  ...
  if (resolvedBinaries.length === 0) {
    console.error(`\ncredit-agents: Platform binary not installed.`);
  ...
  for (let index = 0; index < resolvedBinaries.length; index += 1) {
    ...
    if (result.error) {
      ...
      console.error(`\ncredit-agents: Failed to execute binary.`);
